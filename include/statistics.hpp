    ...
    struct statistics {
        struct builder {
            ...
            void compute_smoothing_statistics(Iterator begin, size_t len)
            {
                ...
                for (size_t i = 0; i < len; ++i, ++begin) {
                    ...
                    for (uint8_t n = 1; n < N; ++n) {
                        ...
                        // context changes
                        if (n != 1 and not ptr.equal_to(prev_ptr, N - n,
                                                                  N - 1)) // N - 1 is excluded from comparison (one-past the end)
                        {
                            m_tmp_stats.combine(n);
                            ++m_num_ngrams[n - 2]; // set num. ngrams of previous order
                        }
                    ...
                    // N-gram case (special): they do not have modified counts, rather their counts are equal to the occurrence in text
            ...
            void build(statistics& stats) {
                ...
                m_unk_prob /= stats.num_ngrams(1); // interpolate with uniform distribution: 1/|vocabulary|

                stats.m_t.swap(m_t);
                stats.m_D.swap(m_D);
                stats.m_total_num_words = m_total_num_words - N + 1;
                stats.m_unk_prob = m_unk_prob;
                std::cerr << "total num. tokens: " << stats.m_total_num_words << std::endl;
            }
        ...
        private:
            ...
            size_t m_vocab_size;
            float m_unk_prob;           // prob of <unk> word, which is backoff(empty) / vocabulary_size
            ...
            void complain(uint8_t n, uint64_t k) {
                std::cerr << "Error: could not calculate Kneser-Ney discounts for "
                          << int(n) << "-grams with adjusted count "
                          << k << " because it was not observed any "
                          << int(n) << "-grams with adjusted count:\n";
                check(n, 1); check(n, 2); check(n, 3);
        ...
        uint64_t total_grams() const {
            return std::accumulate(m_num_ngrams.begin(),
                                   m_num_ngrams.end(),
                                   uint64_t(0));
        }
