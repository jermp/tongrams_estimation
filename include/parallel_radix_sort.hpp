...
namespace tongrams {
    ...
    struct parallel_lsd_radix_sorter {
        parallel_lsd_radix_sorter(uint32_t max_digit,
                                  uint32_t num_digits,
        ...
        void sort(Iterator begin, Iterator end) const
        {
            ...
            for (uint32_t column_index = first_column_index;
                          column_index - first_column_index < m_num_digits;
                        ++column_index)
            {
    ...
    private:
        ...
        void parallel_counting_sort(Iterator begin, Iterator end, uint32_t column_index) const
        {
            std::vector<std::vector<uint32_t>> counts(m_num_threads + 1,
                                                      std::vector<uint32_t>(m_max_digit, 0));
            size_t n = end - begin;
